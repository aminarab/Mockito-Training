...
public class MathApplicationMockitoTester {
	...
	public void testAdd() throws NoSuchFieldException, SecurityException {
		...
		when(calcService.add(10.0, 20.0)).thenReturn(65.00);

		// test the add functionality
		Assert.assertEquals(mathApplication.add(10.0, 20.0), 75.0, 0);
...
//		anyShort()
//		anyString()
		when(calcService.add(anyDouble(), anyDouble())).thenReturn(1.0);
		Assert.assertEquals(mathApplication.add(20.0, 10.0), 11.0, 0);

		reset(calcService);
				
//		any(type);
		when(calcService.add(any(Double.class), any(Double.class))).thenReturn(2.0);
		Assert.assertEquals(mathApplication.add(20.0, 10.0), 12.0, 0);
